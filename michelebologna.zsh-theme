...
GIT_PROMPT='$(if [[ x"$(tmux showenv NOGIT 2>&1)" = x"NOGIT=1" ]]; then printf "#"; else out=$(git_prompt_info)$(git_prompt_status)$(git_remote_status); if [[ -n $out ]]; then printf %s " $white($green$out$white)$reset";fi;fi)'
PROMPT+="$GIT_PROMPT"
